<!DOCTYPE html>
<!-- link to site: https://stuweb.cms.gre.ac.uk/~sm2418r/Enterprise/Homes.php -->
<?php  session_start(); 
require 'mysql.php';
...
/*            background-color: #eaeaea;*/
    ...
    <script>
        function UpVote(PostId) {
            ...
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    document.getElementById("likeBtn-" + PostId).innerHTML = this.responseText;
                }
            };
            xmlhttp.open("GET", "Like.php?q=" + PostId, true);
            xmlhttp.send();
        ...
        function DownVote(PostId) {
            ...
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    document.getElementById("dislikeBtn-" + PostId).innerHTML = this.responseText;
                }
            };
            xmlhttp.open("GET", "Dislike.php?q=" + PostId, true);
            xmlhttp.send();
...
//    $query = "SELECT * from Posts ORDER BY PostID DESC LIMIT  $offset, $no_of_records_per_page";   //WORKING QUERY
    $query = "SELECT PostID, Title, Posts.DepartmentID, Department.Department,      Description, Up_Vote, Down_Vote from Posts
            ...
            <div class="col-sm-4" >
                <div class="card">
                    <a href="Post?PostID='.$row['PostID'].'"><div class="card-body">
                        ...
                        <div class="buttons">
                            <a onclick="UpVote('.$row['PostID'].')" id="likeBtn-'.$row['PostID'].'" class="btn btn-success btn-sm">
                                <span class="glyphicon glyphicon-thumbs-up"></span> Like '.$row["Up_Vote"].'
                            </a>
                            <a onclick="DownVote('.$row['PostID'].')" id="dislikeBtn-'.$row['PostID'].'" class="btn btn-danger btn-sm">
            ...
            <div class="col-sm-4" >
                <div class="card">
                  <a href="Post?PostID='.$row['PostID'].'"> <div class="card-body">
                        ...
                        <div class="buttons" >
                            <a onclick="UpVote('.$row['PostID'].')"  id="likeBtn-'.$row['PostID'].'" class="btn btn-success btn-sm">
                            <span class="glyphicon glyphicon-thumbs-up"></span> Like '.$row["Up_Vote"].'</a>
                            <a onclick="DownVote('.$row['PostID'].')" id="dislikeBtn-'.$row['PostID'].'" class="btn btn-danger btn-sm" >
                                <span class="glyphicon glyphicon-thumbs-up"></span> Disike '.$row["Down_Vote"].'
                            </a> <a id="date" class="date">'.$newDate.'</div>
                        </div>
