    ...
    if ($valid == ""){
        if (isEmailAvailable($email, $link)){
            ...
            if($confirmPass == $pass){
                ...
                $ps =$link->prepare("INSERT INTO Staff (DepartmentID, RoleID, Email, Name, Password, Date_Joined, Last_Logged, Post_Count, Comment_Count) Values(?,?,?,?,?,?,?,?,?)");
                
                // Binding/ setting the parameters for the query
                $ps-> bind_param("iisssssii", $DepartmentID, $RoleID, $Email, $Name, $Password, $Date_Joined, $Last_Logged, $Post_Count, $Comment_Count);
