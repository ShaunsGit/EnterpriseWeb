      ...
      <div class="row">
        ...
        <div class="col-lg-10">
          <?php
           ...
           echo SuccessMessage();
           ?>
          <h1>Number of Post Per Category</h1>
          <table class="table table-striped table-hover">
            ...
            $SrNo = 0;
            global $ConnectingDB;
            $sql = "SELECT c.CategoryID, c.Category, coalesce(oc.Count, 0) as Number_Of_Post from Category c left join ( select CategoryID, count(*) as Count from Posts group by CategoryID ) oc on (c.CategoryID = oc.CategoryID)";
            $stmt=$ConnectingDB->query($sql);
            while ($DataRows=$stmt->fetch()) {
              $CatID = $DataRows["CategoryID"];
                $Desc = $DataRows["Category"];
